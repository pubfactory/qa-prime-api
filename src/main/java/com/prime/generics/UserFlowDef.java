...
public class UserFlowDef extends BaseTest {
     ...
     * 
     * Returns void
     * Function to verify various Email Button Features 
    ...
    public void VerifyVariousEmailButtonFeatures() throws Exception {
        try {
            ...
            Allure.step("Verifying the same article is opened when copy the link from share Link pop up and paste in new tab");
            ...
            Allure.step("Verifying the Share link pop up is Closed after clicking the close button of share link popup");
            // soft.assertAll();
        } catch (Exception e) {
            e.getStackTrace();
        } finally {
    ...
    public void selectIssueDDAndVerifyCurrentIssueOnMostRecentVolumeList() throws Exception {
        ...
        soft.assertEquals(largestVolumeNumber, latestVolumeNum, "Verifying the latest volume is displayed on the select issue drop down at the top.");
        ...
        soft.assertEquals(largestIssueNumber, latestIssueNum, "verifying the recent Issue is displayed under Recent Volume on the select Issue DD at the top.");
        ...
        url = BaseTest.properties.getProperty(application);
        soft.assertEquals(
                Helper.INSTANCE.removeBasicAuthFromURLFromWebAppURL(basePage.getURLFromWebPage()), Helper.INSTANCE.removeBasicAuthFromURLFromWebAppURL(url) + "view/journals/anpr/" + Integer.toString(latestVolumeNum)
                        + "/" + Integer.toString(latestIssueNum) + "/anpr." + Integer.toString(latestVolumeNum) + ".issue-" + Integer.toString(latestIssueNum) + ".xml",
    ...
    public void verifyAccessIconIsLockedWhenIamNotLoggedIn() throws Exception {
        ...
        soft.assertEquals(accessIcon, "Restricted access", "Verifying the access icon is locked when user is not logged in");
    ...
    public void verifyTheFirstArticleLoadsCorrectlyAndClickedLinkMatchesTitleOfLoadedArticle() throws Exception {
        ...
        soft.assertEquals(actualTitle, expectedTitle, "Verifying user clicked link matches the title of the loaded article.");
        ...
        soft.assertEquals(actualresult, "true", "Verifying user land on abstract tab or the PDF Preview tab if there is no abstract tab.");
    ...
    public void VerifyAllArticleAreFromTheIssueWhileNavigatingTheNextPrevControl() throws Exception {
        articleCitationPage = BasePage.initialize(WebDriverManager.getDriver(), ArticleCitationPage.class);
        soft.assertEquals(articleCitationPage.verifyArticleAreFromTheIssueWhileNavigatingTheNextArticle(titleList), true, "Verifying all the Article are from The Issue while navigating the Next control.");
        Allure.step("Verifying all the Article are from The Issue while navigating the Next control.");

        soft.assertEquals(articleCitationPage.verifyArticleAreFromTheIssueWhileNavigatingThePrevArticle(titleList), true, "Verifying all the Article are from The Issue while navigating the Prev control.");
    ...
    public void verifyFulltextIsNavigableThroughArticleContent() throws Exception {
        ...
        soft.assertEquals(articleCitationPage.getTheAriaExpandedValueForarticleInformationTab(), "true", "Verifying user on article information tab");
        Allure.step("Verifying user on article information tab");
        articleCitationPage.clickOnResultsAticleContentLinkOnArticlePage();
        soft.assertEquals(articleCitationPage.getTheAriaExpandedValueForFulltextTab(), "true", "Verifying once user clicks on the Results article content link, user navigates on Full Text tab");
        ...
        soft.assertEquals(articleCitationPage.getTheAriaExpandedValueForFulltextTab(), "true", "Verifying once user clicks on the Conclusion article content link, user navigates on Full Text tab");
        Allure.step("Verifying once user clicks on the Conclusion article content link, user navigates on Full Text tab");
        articleCitationPage.clickOnArticleInformationTabOnArticlePage();
        soft.assertEquals(articleCitationPage.getTheAriaExpandedValueForarticleInformationTab(), "true", "Verifying user on article information tab");
        Allure.step("Verifying user on article information tab");
        articleCitationPage.clickOnReferenceAticleContentLinkOnArticlePage();
        soft.assertEquals(articleCitationPage.getTheAriaExpandedValueForFulltextTab(), "true", "Verifying once user clicks on the References article content link, user navigates on Full Text tab");
        Allure.step("Verifying once user clicks on the References article content link, user navigates on Full Text tab");
    }

    /**
     * Function to verify all figures are loading in the Figure tab in the article page
     * @throws Exception
     */
    ...
    public void verifyAllFiguresAreLoadingInTheFigureTab() throws Exception {
        ...
        soft.assertEquals(articleCitationPage.verifyAllFiguresAreLoadingInTheFigureTabOnArticlePage(), true, "Verifying all figures are loaded in figures tab on article page.");
        Allure.step("Verifying all figures are loaded in figures tab on article page.");
    }

    /**
     * 
     * Function to verify that Figure Opens in Power point and user is able to download.
     * @throws Exception
     */
    ...
    public void verifyFigureOpenInPowePointOnceClicksOnItAndAbleToDownloadTheFigureInPPTFormatIfClicksOnDownloadButton() throws Exception {
        try {
            ...
            soft.assertEquals(articleCitationPage.verifyFiguresDiplayedInPPTFormat(), true, "verifying figures is diplayed in PPT format");
            Allure.step("Verifying figures is diplayed in PPT format");
            articleCitationPage.clickOnNextButtonForSlidePPT();
            soft.assertEquals(articleCitationPage.getTheSrcAttributeValueOfImageOrFigure(), Helper.INSTANCE.removeBasicAuthFromURLFromWebAppURL(url) + "view/journals/anpr/67/2/full-i0003-3006-67-2-72-f02.png",
            ...
            soft.assertEquals(articleCitationPage.getTheSrcAttributeValueOfImageOrFigure(), Helper.INSTANCE.removeBasicAuthFromURLFromWebAppURL(url) + "view/journals/anpr/67/2/full-i0003-3006-67-2-72-f01.png",
            ...
            soft.assertEquals(articleCitationPage.verifyFiguresIsDownloadedInPPTFormat(".pptx"), true, "Verifying the figures is downloaded in PPT format.");
    ...
    public void verifyCitationFunctionalityAndSelectEachFormatOfCitationAndCopyToClipBoardAndMakeSureItMatchesTheCitationPreview() throws Exception {
        try {
            ...
            // button
            soft.assertEquals(articleCitationPage.getPreviewExportCitationPopUpHeaderText(), testData.get("popupheader").toString(), "Verifying the Preview Export Citation popup is displayed");
            ...
            soft.assertEquals(articleCitationPage.verifyRISButtonIsPresentOnPreviewExportCitationOnPopup(), true, "Verifying the RIS Button is present under Export citation section on Preview Export Citation PopUp");
            Allure.step("Verifying the RIS Button is present under Export citation section on Preview Export Citation PopUp");
            soft.assertEquals(articleCitationPage.verifyBIBButtonIsPresentOnPreviewExportCitationOnPopup(), true, "Verifying the BIB Button is present under Export citation section on Preview Export Citation PopUp");
            Allure.step("Verifying the BIB Button is present under Export citation section on Preview Export Citation PopUp.");
            soft.assertEquals(articleCitationPage.verifyENWButtonIsPresentOnPreviewExportCitationOnPopup(), true, "Verifying the ENW Button is present under Export citation section on Preview Export Citation PopUp");
            Allure.step("Verifying the ENW Button is present under Export citation section on Preview Export Citation PopUp");
            ...
            soft.assertEquals(articleCitationPage.getExportCitationFormatLabels(testData.get("risbutton").toString()).toString(), expRISCitationLabels.toString(), "Verifying RIS button Labels on Export Ciatation Popup");
            Allure.step("Verifying RIS button Labels on Export Ciatation Popup");
            soft.assertEquals(articleCitationPage.getExportCitationFormatLabels(testData.get("bibbutton").toString()).toString(), expBIBCitationLabels.toString(), "Verifying BIB button Labels on Export Ciatation Popup");
            Allure.step("Verifying BIB button Labels on Export Ciatation Popup");
            soft.assertEquals(articleCitationPage.getExportCitationFormatLabels(testData.get("enwbutton").toString()).toString(), expENWCitationLabels.toString(), "Verifying ENW button Labels on Export Ciatation Popup");
            ...
            Allure.step("Verifying Preview Export Citation PopUp Close Button is present on Preview Export Citation PopUp");
            ...
            // popup
            soft.assertEquals(articleCitationPage.verifyCopyToClipBoardButtonIsPresentOnPreviewExportCitationPopup(), true, "Verifying the Copy to ClipBoard button is present on preview Export Citation popup");
            ...
            if (headLess.equalsIgnoreCase("N")) {
                soft.assertEquals(articleCitationPage.verifyChakraToastMessageIsDisplayedAfterClickingOnCopyToClipBoardButton(), true,
                        "Verifying the chakra toast message is displyed after clinking on the copy to clipboard button on Export Citation popup");
                Allure.step("Verifying the chakra toast message is displyed after clinking on the copy to clipboard button on Export Citation popup");
                articleCitationPage.clickOnToastMessagePopupCloseButton();
            } else {
                soft.assertEquals(articleCitationPage.verifyChakraToastMessageIsDisplayedAfterClickingOnCopyToClipBoardButtonInHeadless(), true,
                        "Verifying the chakra toast message is displyed after clinking on the copy to clipboard button on Export Citation popup");
                Allure.step("Verifying the chakra toast message is displyed after clinking on the copy to clipboard button on Export Citation popup");
            }
            ...
            soft.assertEquals(BaseTest.verifyStringContainsSpecificWord(abbreviatedTitleAPA, pastedAPAValued), true, "Verifying the correct format value is copied and pasted");
            ...
            soft.assertEquals(BaseTest.verifyStringContainsSpecificWord(abbreviatedTitleAMA, pastedAMAValued), true, "Verifying the correct format value is copied and pasted");
            Allure.step("Verifying the correct format value is copied and pasted.");
            driver.get(articleUrl);
            // soft.assertAll();
        } catch (Exception e) {
    ...
    public void verifyShareButtonIsPresentAndEachOptionPromptsTheUSerToLogInInToTheRespectiveService() throws Exception {
        try {
            driver.navigate().refresh();
            articleCitationPage = BasePage.initialize(WebDriverManager.getDriver(), ArticleCitationPage.class);
            soft.assertEquals(articleCitationPage.getShareButtonTextOnArticlePage(), "Share", "Verifying the share button is present on article page");
            Allure.step("Verifying the share button is present on article page");
            articleCitationPage.clickOnShareButtonOnArticlePage();
            soft.assertTrue(articleCitationPage.verifySharingPlatformDataNetworkUnderShareButton("facebook"), "Verifying data-network for facebook attribute is present");
            ...
            soft.assertTrue(articleCitationPage.verifySharingPlatformDataNetworkUnderShareButton("linkedin"), "Verifying data-network for linkedin attribute is present");
            ...
            soft.assertTrue(articleCitationPage.verifySharingPlatformDataNetworkUnderShareButton("twitter"), "Verifying data-network for twitter attribute is present");
    ...
    public void verifyPDFButonAvailableAndDownloadPDF() throws Exception {
        try {
            ...
            soft.assertEquals(pdfPage.verifyPDFButtonPresentOnArticlePage(), true, "Verifying PDF Button is present on the article page");
            ...
            // PDF Tab
            soft.assertEquals(pdfPage.getDefaultPDFZoomValueInInlinePDFTab(), testData.get("defaultzoomvalue").toString(), "Verifying Default zoom value in Inline PDF Tab on article page");
            ...
            soft.assertEquals(BaseTest.verifyStringContainsSpecificWord(articleHeader, partialArticleHeader), true, "Verifying that same article is displayed in PDF preview in inline PDF tab");
            ...
            soft.assertEquals(pdfPage.verifyWatermarkIsPresentOnPreviewInPDFTabOnArticlePage(applicationName), true, "Verifying that watermark is present on pdf in Pdf preview in Inline tab on the articla page");
    ...
    public void verifyGooglescholarAndPubmedSectionFunctionality() {
        try {
            driver.navigate().refresh();
            articleCitationPage = BasePage.initialize(WebDriverManager.getDriver(), ArticleCitationPage.class);
            soft.assertEquals(articleCitationPage.verifyGoogleScholarButtonPresentOnArticlePage(), true, "Verifying the Google Scholar button is present at right hand side on Artical page");
            ...
            soft.assertEquals(affiliationBlockAuthor.toString(), googleScholarAuthor.toString(), "Verifying the similar author affiliation block authors is present in the Google Scholar section on Artical page");
            Allure.step("Verifying the similar author affiliation block authors is present in the Google Scholar section on Artical page.");

            // Verifying Similar article hyper link present in Google scholar section and
            // similar article open when click on it
            soft.assertEquals(articleCitationPage.verifySimilarArtcileHyperLinkPresentOnArticlePage(), true, "Verifying the Similar Artcile Hyper Link is present in the Google Scholar section on Artical page");
            Allure.step("Verifying the Similar Artcile Hyper Link is present in the Google Scholar section on Artical page.");
            ...
            articleCitationPage = BasePage.initialize(WebDriverManager.getDriver(), ArticleCitationPage.class);
            soft.assertEquals(articleCitationPage.verifyPubmedButtonPresentOnArticlePage(), true, "Verifying the PubMed button is present at right hand side on Artical page");
            ...
            soft.assertEquals(affiliationBlockAuthorPub.toString(), pubMedAuthor.toString(), "Verifying the similar author affiliation block authors is present in the PubMed section on Artical page");
            Allure.step("Verifying the similar author affiliation block authors is present in the PubMed section on Artical page.");
            ...
            Helper.INSTANCE.switchToWindowTab(1);
            soft.assertTrue(BaseTest.verifyTextInURL(authorFirstNamePub), "Verifying Same article open in new PubMed site when clicked on author in PubMed section on article page");
            Allure.step("Verifying Same article open in new PubMed site when clicked on author in PubMed section on article page.");
    ...
    public void verifyThatTheAutoLaunchNewEmailFunctionality() throws Exception {
        ...
        soft.assertEquals(articleCitationPage.verifyEmailElementIsPresentUnderArticleInformationTabOnArticlePage(), true,
                "Verifying the Email address is present under contributor Notes section in Article information tab Article Page");
        Allure.step("Verifying the Email address is present under contributor Notes section in Article information tab Article Page.");

        // Verifying the Email address is hyper link
        soft.assertEquals(articleCitationPage.verifyEmailAddressIsHyperLinkUnderArticleInformationTab(), true, "Verying the Email address is hyper link under Article Information tab on article page");
    ...
    public void verifyContentMetaDataServiceInReferences() {
        try {
            ...
            soft.assertEquals(testData.get("searchpubmed").toString(), articleCitationPage.getFirstSearchPubMedTextUnderReferecesSectionOnArticlePage(),
            ...
            Helper.INSTANCE.switchToWindowTab(1);
            soft.assertTrue(BaseTest.verifyTextInURL("pubmed"), "Verifying pubmed site is opened in new tab when user clicked on the search Pubmed link under references section on Article page");
            Allure.step("Verifying pubmed site is opened in new tab when user clicked on the search Pubmed link under references section on Article page.");
            ...
            // on it.
            soft.assertEquals(testData.get("searchgooglescholar").toString(), articleCitationPage.getFirstSearchGoogleScholarTextUnderReferecesSectionOnArticlePage(),
            ...
            Helper.INSTANCE.switchToWindowTab(1);
            soft.assertTrue(BaseTest.verifyTextInURL("scholar"), "Verifying Google Scholar site is opened in new tab when user clicked on the search google scholar link under references section on Article page");
            Allure.step("Verifying Google Scholar site is opened in new tab when user clicked on the search google scholar link under references section on Article page.");
            soft.assertEquals(articleCitationPage.verifyGoogleScholarSiteIsLoaded(), true, "Verifying Google Scholar site is loaded");
    ...
    public void verifyThatDOIShouldBeLinkAndHandlingDOIRedirectFunctionality() throws Exception {
        try {
            ...
            soft.assertEquals(articleCitationPage.verifyDOIMetaDataIsLink(), true, "Verifying the DOI META data is Link");
            ...
            soft.assertEquals(DOIArticle, DOIMainSite, "Verifying that clicking on DOI value in the current environment will take the user to the live environment and should render the same DOI value");
            Allure.step("Verifying that clicking on DOI value in the current environment will take the user to the live environment and should render the same DOI value.");
            ...
            soft.assertEquals(actualArticleTitlewithoutSlash, expectedArticleTitle, "Verifying the proper content page is displayed if the URL contains only the actual DOI value.");
            ...
            soft.assertEquals(actualArticleTitlewithSlash, expectedArticleTitle, "Verifying that the correct content page is displayed if an extra path is added after the DOI.");
    ...
    public void verifyThatTheCitationLinkFunctionalityIsWorkingFine() throws Exception {
        try {
            ...
            soft.assertEquals(articleCitationPage.verifyCitationsLinkIsPresentInGoofglrScholarSectionOnArticlePage(), true, "Verifying the Citations link is present in google scholar section on Article page");
            ...
            soft.assertEquals(Helper.INSTANCE.verifyingNewTabIsOpen(), true, "Verifying the new tab opened after clicking on Citation link");
            ...
            // page.
            soft.assertEquals(articlePageURL, googleScholarPageURL, "Verifying that the new tab is opened and it contains the URL of the article page.");
