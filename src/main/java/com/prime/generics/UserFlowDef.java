...
public class UserFlowDef extends BaseTest {
    ...
    public void selectIssueDDAndVerifyCurrentIssueOnMostRecentVolumeList() throws Exception {
        ...
        BaseTest.assertEquals(WebDriverManager.getDriver(), Helper.INSTANCE.compareIntValue(recentVolumeNum, secondRecentVolumeNum), true, "verifying the recent volume is displayed on the select issue DD at the top.");
        ...
        if (issuePage.getTotalIssueListUnderVolumeFromSelectIssueDDOnJournalPage(recentVolume).size() > 1) {
            System.out.println("size: " + issuePage.getTotalIssueListUnderVolumeFromSelectIssueDDOnJournalPage(recentVolume).size());
            String secondIssue = issuePage.getTotalIssueListUnderVolumeFromSelectIssueDDOnJournalPage(recentVolume).get(1);
        ...
        BaseTest.assertEquals(WebDriverManager.getDriver(), Helper.INSTANCE.compareIntValue(recentIssueNum, secondIssueNum), true,
        ...
        url = BaseTest.properties.getProperty(application);
        BaseTest.assertEquals(
                WebDriverManager.getDriver(), Helper.INSTANCE.removeBasicAuthFromURLFromWebAppURL(basePage.getURLFromWebPage()), Helper.INSTANCE.removeBasicAuthFromURLFromWebAppURL(url) + "view/journals/anpr/"
                        + Integer.toString(recentVolumeNum) + "/" + Integer.toString(recentIssueNum) + "/anpr." + Integer.toString(recentVolumeNum) + ".issue-" + Integer.toString(recentIssueNum) + ".xml",
    ...
    public void selectIssueDDAndVerifyCurrentIssueOnMostRecentVolumeList_VEENA() throws Exception {
        ...
        // Verify the most recent volume list is displayed at the top.
        BaseTest.assertEquals(WebDriverManager.getDriver(), largestVolumeNumber, latestVolumeNum, "Verifying the latest volume is displayed on the select issue drop down at the top.");
        ...
        BaseTest.assertEquals(WebDriverManager.getDriver(), largestIssueNumber, latestIssueNum, "verifying the recent Issue is displayed under Recent Volume on the select Issue DD at the top.");
    ...
    public void verifyEachListingHasTitleContributorDOIAndAbstractButton() throws Exception {
        ...
        BaseTest.assertEquals(WebDriverManager.getDriver(), totalTitle, contributor, "Verifying each listing has a contributor(s).");
        BaseTest.assertEquals(WebDriverManager.getDriver(), totalTitle, DOI, "Verifying each listing has a DOI.");
        BaseTest.assertEquals(WebDriverManager.getDriver(), totalTitle, abstractButton, "Verifying each listing has a Abstract Button.");
    ...
    public void verifyAccessIconIsLockedWhenIamNotLoggedIn() throws Exception {
        ...
        BaseTest.assertEquals(WebDriverManager.getDriver(), signInText, "Sign in", "Verifying the user has is not logged In.");
        BaseTest.assertEquals(WebDriverManager.getDriver(), accessIcon, "Restricted access", "Verifying the access icon is locked when user is not logged in");
    ...
    public void verifyTheFirstArticleLoadsCorrectlyAndClickedLinkMatchesTitleOfLoadedArticle() throws Exception {
        ...
        BaseTest.assertEquals(WebDriverManager.getDriver(), actualTitle, expectedTitle, "Verifying user clicked link matches the title of the loaded article.");
        String actualresult = articleCitationPage.VerifyByDefaultAbstractTabClickedOrPDFTabIfNoAbstract();
        BaseTest.assertEquals(WebDriverManager.getDriver(), actualresult, "true", "Verifying user land on abstract tab or the PDF Preview tab if there is no abstract tab.");
