...
public class CitationLinkTest extends BaseTest {
	...
	public void VerifyThatTheCitationLinkFunctionalityIsWorkingFine() throws Exception {
		try {
			...
			BaseTest.assertEquals(WebDriverManager.getDriver(), Helper.INSTANCE.verifyingNewTabIsOpen(), true,
			...
			BaseTest.assertTrue(WebDriverManager.getDriver(), BaseTest.verifyTextInURL("https://scholar.google.com/scholar?q=link"), "verifying the google scholar search result is displayed with following search query: https://scholar.google.com/scholar/?=link");
			...
			BaseTest.assertEquals(WebDriverManager.getDriver(),articlePageURL,googleScholarPageURL,
					"Verifying that the new tab is opened and it contains the URL of the article page.");
