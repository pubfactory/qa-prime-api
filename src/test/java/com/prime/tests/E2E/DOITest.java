...
public class DOITest extends BaseTest {
    ...
    @Severity(SeverityLevel.BLOCKER)
    @Test(groups = {"DOIExtraPath"}, enabled = true, retryAnalyzer = Retry.class, description = "1730101 - verify that handling DOI redirect functionality")
    @Story("PRIME-3620")
    public void verifyThatHandlingDOIRedirectFunctionality() throws Exception {
        ...
        driver.get(url + withoutExtraSlashDOI);
        ...
        BaseTest.assertEquals(WebDriverManager.getDriver(), actualArticleTitlewithoutSlash, expectedArticleTitle, "Verifying the proper content page is displayed if the URL contains only the actual DOI value.");
        ...
        BaseTest.assertEquals(WebDriverManager.getDriver(), actualArticleTitlewithSlash, expectedArticleTitle, "Verifying that the correct content page is displayed if an extra path is added after the DOI.");
