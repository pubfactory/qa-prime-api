...
public class DiscoveryWidgetTest extends BaseTest {
	...
	@Test(groups = {
			"Discovery Widget" }, enabled = true, retryAnalyzer = Retry.class, description = "65 - Verify various Email button features")
	@Story("EPIC-973")
	...
	public void VerifyVariousEmailButtonFeatures() throws Exception {
		try {
			...
			BaseTest.assertEquals(WebDriverManager.getDriver(), articleCitationPage.getShareLinkPopupHeader(),
...
//					"Verifying the Link copied message dispayed after clicking on copy link button on share link pop up on Article page");
			...
			BaseTest.assertEquals(WebDriverManager.getDriver(), articleText, articleTextInNewTab,
...
//	        Helper.INSTANCE.switchToWindowTab(0);
	...
	@Severity(SeverityLevel.BLOCKER)
	@Test(groups = {
			"Discovery Widget" }, enabled = true, retryAnalyzer = Retry.class, description = "66 - Verify Google Scholar Button features")
	@Story("EPIC-973")
	public void VerifyGoogleScholarButtonFeatures() throws Exception {
		try {
			...
			articleCitationPage = BasePage.initialize(WebDriverManager.getDriver(), ArticleCitationPage.class);
			BaseTest.assertEquals(WebDriverManager.getDriver(),
					articleCitationPage.verifyGoogleScholarButtonPresentOnArticlePage(), true,
			...
			BaseTest.assertEquals(WebDriverManager.getDriver(), affiliationBlockAuthor.toString(),
					googleScholarAuthor.toString(),
			...
			BaseTest.assertEquals(WebDriverManager.getDriver(),
					articleCitationPage.verifySimilarArtcileHyperLinkPresentOnArticlePage(), true,
			...
			BaseTest.assertTrue(WebDriverManager.getDriver(), BaseTest.verifyTextInURL(authorFirstName),
	...
	@Severity(SeverityLevel.BLOCKER)
	@Test(groups = {
			"Discovery Widget" }, enabled = true, retryAnalyzer = Retry.class, description = "67 - Verify PubMed button features and Almetric page features")
	@Story("EPIC-973")
	public void VerifyPubMedButtonFeaturesAndAlmetricPageFeatures() throws Exception {
		...
		articleCitationPage = BasePage.initialize(WebDriverManager.getDriver(), ArticleCitationPage.class);
		BaseTest.assertEquals(WebDriverManager.getDriver(),
				articleCitationPage.verifyPubmedButtonPresentOnArticlePage(), true,
		...
		BaseTest.assertEquals(WebDriverManager.getDriver(),affiliationBlockAuthor.toString(), pubMedAuthor.toString(), "Verifying the similar author affiliation block authors is present in the PubMed section on Artical page");
