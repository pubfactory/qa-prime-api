...
public class AddCiteddByTest extends BaseTest {
    ...
    @Test(groups = {"astmh"}, enabled = true, retryAnalyzer = Retry.class, description = "104  - Verify that the Add Cited By Functionality")
    @Story("EPIC-2692")
    public void VerifyThattheAddCitedByFunctionality() throws Exception {
        try {
            ...
            BaseTest.assertEquals(WebDriverManager.getDriver(), articleCitationPage.getNumberOfCitationTextOnArticlePae().trim(), testData.get("numbercitationele").toString(),
            ...
            if (articleCitationPage.getCountOfNumberOfCitationOnArticlePage() > 0) {
                BaseTest.assertEquals(WebDriverManager.getDriver(), articleCitationPage.getCitedByTextOnArticlePage().trim(), testData.get("citedby").toString(),
                        "Verifying the Cited By element is present on Article page");
                ...
                BaseTest.assertEquals(WebDriverManager.getDriver(), articleCitationPage.verifyCitationIndexIsNotPresentInCitedByOnArticlePage(), true,
                ...
                if (articleCitationPage.verifyPaginationStyleWillBeAStringOfNumbersInCitedByOnArticlePage() == true) {
                    BaseTest.assertEquals(WebDriverManager.getDriver(), articleCitationPage.verifyPaginationStyleWillBeAStringOfNumbersInCitedByOnArticlePage(), true,
                            "Verifying the pagination is a string of numbers, matching search pagination.");
                    articleCitationPage.clickOnSecondPaginationLinkInCitedByElementOnArticlePage();
                    int totalResultDiplayedInInCitedByElemen = articleCitationPage.getAppropriateCitationResultCountInCitedByElementOnArticlePage();
                    String defaultText = articleCitationPage.getDefaultYouAreLookingForTextInCitedByElement();
                    int startNumber = articleCitationPage.getStartingAndEndingResultTextOfYouAreLookingInCitedByElement(1);
                    int endNumber = articleCitationPage.getStartingAndEndingResultTextOfYouAreLookingInCitedByElement(totalResultDiplayedInInCitedByElemen);
                    BaseTest.assertEquals(WebDriverManager.getDriver(), defaultText, "You are looking at " + startNumber + "-" + endNumber + " of " + totalCitationNumber + "citations",
                    ...
                    // component.
                    BaseTest.assertEquals(WebDriverManager.getDriver(), articleCitationPage.verifyPagInationInTopInCitedByElementIsPresentOnArticlePage(), true,
                            "Verifying Verifying the pagInation in top of cited by element is present on article page.");
                    BaseTest.assertEquals(WebDriverManager.getDriver(), articleCitationPage.verifyPagInationInBottomInCitedByElementIsPresentOnArticlePage(), true,
                ...
                BaseTest.assertEquals(WebDriverManager.getDriver(), articleCitationPage.getCrossRefTextInCitedByElement(), testData.get("crossref").toString(),
                ...
                BaseTest.assertEquals(WebDriverManager.getDriver(), articleCitationPage.verifyDOIUserIsPresentInNewTab(), true,
                ...
                int totalResultDiplayedInInCitedBy = articleCitationPage.getAppropriateCitationResultCountInCitedByElementOnArticlePage();
                String defaultText = articleCitationPage.getDefaultYouAreLookingForTextInCitedByElement();
                int startNumberdefault = articleCitationPage.getStartingAndEndingResultTextOfYouAreLookingInCitedByElement(1);
                int endNumberDefault = articleCitationPage.getStartingAndEndingResultTextOfYouAreLookingInCitedByElement(totalResultDiplayedInInCitedBy);
                BaseTest.assertEquals(WebDriverManager.getDriver(), defaultText, "You are looking at " + startNumberdefault + "-" + endNumberDefault + " of " + totalCitationNumber + "citations",
                        "Verifying the You are looking at " + startNumberdefault + " -" + endNumberDefault + " of " + totalCitationNumber + " citation is displayed in Cited by component");
            }
        } catch (Exception e) {
            e.getStackTrace();
        } finally {
